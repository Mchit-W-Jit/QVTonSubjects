...
def create_issue(sura_name, section_number, riwaya, label):
    ...
    #Sleeping 10s to avoid GraphQL: was submitted too quickly (createIssue) error
    ...
    print(f"gh issue create -t \"{title}\" -b \"{body}\" -l \"{label},{riwaya},audacity,audio\" -a @me && sleep 10")
    ...
    #command = ["gh", "issue", "create", "-t", title, "-b", body, "-l", label, "-a @me"]
    print(f"gh issue create -t \"{title}\" -b \"{body}\" -l \"{label},{riwaya},thumbnail\" -a @me && sleep 10")
    #subprocess.run(command, check=True)
    ...
    #command = ["gh", "issue", "create", "-t", title, "-b", body, "-l", label, "-a @me"]
    print(f"gh issue create -t \"{title}\" -b \"{body}\" -l \"{label},{riwaya},text\" -a @me && sleep 10")
    #subprocess.run(command, check=True)
